...
const downloadResumeButton = document.getElementById(
  "download-resume"
) as HTMLButtonElement;
...
function generatePersonalInfoSectionWithEdit(
  name: string,
  email: string,
  phone: string,
...
document.addEventListener("click", (event) => {
  ...
  if (target.classList.contains("edit-button")) {
    ...
    if (parentSection) {
      const saveButton = parentSection.querySelector(
        ".save-button"
      ) as HTMLButtonElement;
      const contentElement = parentSection.querySelector(
        ".editable-content"
      ) as HTMLElement;
  ...
  if (target.classList.contains("save-button")) {
    ...
    const parentSection = button.closest(".editable-section");
    if (parentSection) {
      const editButton = parentSection.querySelector(
        ".edit-button"
      ) as HTMLButtonElement;
      const contentElement = parentSection.querySelector(
        ".editable-content"
